...
CWD=$PWD
ORIGINAL_DATA_PATH=${CWD}/data/RCV1
OUTPUT_BASE=${CWD}/data/RCV1/MLC2SEQ
IS_SWAP=True    # sawp train and test split if true

if [ ! -d "$ORIGINAL_DATA_PATH/CD1" ] || [ ! -d "$ORIGINAL_DATA_PATH/CD2" ]; then
    echo "Make sure you have RCV1 CD1 and CD2 directories under ${ORIGINAL_DATA_PATH}."
    exit 1
fi
...
echo "Uncompress zipped files.."
find ${ORIGINAL_DATA_PATH} \( -path "*/CD1/*" -o -path "*/CD2/*" \) -type f -name '*.zip' -print0 | while IFS= read -r -d '' file
do 
    file_no_ext=${file%%.*}
    file_base="`basename ${file_no_ext}`" 
    if [[ $file_base =~ $re ]] ; then
...
python extract_plain_rcv1_dataset.py \
    --src_dir ${TMP_DIR} \
...
VAD_SIZE=$(expr ${PREV_TRD_NUM_LINE} / 10)
